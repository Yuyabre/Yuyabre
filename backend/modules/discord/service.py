...
class DiscordService:
    ...
    def __init__(self):
        ...
        token_preview = f"{self.bot_token[:10]}..." if self.bot_token and len(self.bot_token) > 10 else "None"
        
        logger.info(f"Discord bot token configured: {token_configured} (preview: {token_preview})")
        ...
        if self.bot_token:
            try:
                ...
                logger.debug(f"Discord intents configured: message_content={intents.message_content}, messages={intents.messages}")
                ...
                logger.error("=" * 60)
                logger.error("✗ DISCORD INITIALIZATION FAILED: discord.py library not installed")
            ...
            logger.warning("   Set DISCORD_BOT_TOKEN environment variable to enable Discord integration")
            logger.warning("=" * 60)
    ...
    def _setup_event_handlers(self):
        ...
        async def on_ready():
            ...
            logger.info(f"   Bot User: {bot_user} (ID: {bot_user.id if bot_user else 'unknown'})")
            ...
            if guild_count > 0:
                ...
                logger.warning("   ⚠ Bot is not in any servers. Invite the bot to your server.")
        ...
        async def on_message(message):
            ...
            if message.author == self._bot_instance.user:
                logger.debug(f"Discord: Ignoring message from bot itself (message ID: {message.id})")
                return
            
            logger.info(f"Discord: Received message from {message.author.name} in channel {message.channel.id}")
            logger.debug(f"   Message ID: {message.id}")
            logger.debug(f"   Channel: {message.channel.name if hasattr(message.channel, 'name') else 'unknown'}")
            logger.debug(f"   Content preview: {message.content[:100] if message.content else '(empty)'}")
            
            # Forward message to webhook endpoint
            try:
                await self._forward_message_to_webhook(message)
            except Exception as e:
                logger.error(f"Discord: Failed to forward message to webhook: {e}", exc_info=True)
        ...
        async def on_reaction_add(reaction, user):
            ...
            if user == self._bot_instance.user:
                logger.debug(f"Discord: Ignoring reaction from bot itself (emoji: {reaction.emoji})")
                return
            
            logger.info(f"Discord: Reaction added by {user.name} - {reaction.emoji} on message {reaction.message.id}")
            logger.debug(f"   User ID: {user.id}, Channel ID: {reaction.message.channel.id}")
            
            # Forward reaction to webhook endpoint
            try:
                await self._forward_reaction_to_webhook(reaction, user, action="add")
            except Exception as e:
                logger.error(f"Discord: Failed to forward reaction to webhook: {e}", exc_info=True)
        ...
        async def on_reaction_remove(reaction, user):
            ...
            # Ignore reactions from the bot itself
            if user == self._bot_instance.user:
                logger.debug(f"Discord: Ignoring reaction removal from bot itself (emoji: {reaction.emoji})")
                return
            
            logger.info(f"Discord: Reaction removed by {user.name} - {reaction.emoji} on message {reaction.message.id}")
            logger.debug(f"   User ID: {user.id}, Channel ID: {reaction.message.channel.id}")
            ...
            try:
                ...
                logger.error(f"Discord: Failed to forward reaction removal to webhook: {e}", exc_info=True)
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"✓ Discord message {message.id} forwarded to webhook successfully")
                else:
                    logger.warning(f"✗ Failed to forward Discord message to webhook")
                    logger.warning(f"   Status code: {response.status_code}")
                    logger.warning(f"   Response: {response.text[:200]}")
        except httpx.TimeoutException:
            logger.error(f"✗ Discord webhook request timed out for message {message.id}")
        except httpx.RequestError as e:
            logger.error(f"✗ Discord webhook request failed for message {message.id}: {e}")
        except Exception as e:
            logger.error(f"✗ Error forwarding Discord message to webhook: {e}", exc_info=True)
    ...
    async def _forward_reaction_to_webhook(self, reaction, user, action: str = "add"):
        ...
        logger.debug(f"   Emoji: {reaction.emoji}, Message ID: {reaction.message.id}, User: {user.name}")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"✓ Discord reaction ({action}) forwarded to webhook successfully")
                else:
            ...
            logger.error(f"✗ Error forwarding Discord reaction to webhook: {e}", exc_info=True)
    ...
    async def start_bot(self):
        ...
        token_preview = f"{self.bot_token[:10]}..." if len(self.bot_token) > 10 else "***"
    ...
    async def send_message_to_channel(
        ...
        *,
        ...
        message_preview = message.strip().splitlines()[0][:120] if message.strip() else ""
    ...
    async def send_to_household(
        ...
        *,
        ...
        if not household.discord_channel_id and not webhook_url:
            logger.warning(f"Household {household_id} has no Discord channel or webhook configured")
            return {}
        ...
        if (message_id is None) and webhook_url:
            logger.info("Falling back to Discord webhook for message delivery (interactive buttons unavailable)")
    ...
    async def _send_message_via_webhook(
        ...
        try:
            ...
            logger.error(f"Unexpected error sending Discord webhook message: {e}", exc_info=True)
            return False
    ...
    async def _handle_generic_button_response(
        ...
        try:
            ...
            logger.debug("Discord: Unable to defer interaction response (possibly already deferred)")
        
        discord_user_id = str(interaction.user.id)
        user = await User.find_one(User.discord_user_id == discord_user_id)
        
        if not user:
            logger.error(f"Discord user {discord_user_id} not linked to any system user")
            try:
        ...
        if initiated_by_user_id:
            try:
                ...
                logger.warning(f"Failed to relay Discord response to agent conversation: {e}", exc_info=True)
    ...
    async def _ensure_bot_ready(self, max_wait_seconds: int = 30) -> bool:
        ...
        logger.info(f"Discord bot not ready yet, waiting up to {max_wait_seconds} seconds...")
        import asyncio
        ...
        for i in range(max_iterations):
            ...
            if self._bot_instance.is_ready():
                logger.info(f"Discord bot became ready after {i * wait_interval:.1f} seconds")
                return True
            
            # Log progress every 5 seconds
            if i > 0 and i % 10 == 0:
                logger.debug(f"Still waiting for Discord bot... ({i * wait_interval:.1f}s elapsed)")
        
        logger.error(f"Discord bot did not become ready within {max_wait_seconds} seconds")
        logger.error(f"Bot state: is_ready()={self._bot_instance.is_ready()}, user={self._bot_instance.user}")
        return False
    ...
    async def send_group_order_notification(
        ...
        if not self.is_configured():
            logger.error("✗ Discord service not configured, cannot send group order notification")
            logger.error("   Check: DISCORD_BOT_TOKEN environment variable is set")
            return False
        ...
        try:
            ...
            if not await self._ensure_bot_ready(max_wait_seconds=30):
                logger.error("✗ Discord bot is not ready, cannot send group order notification")
            ...
            if not household.discord_channel_id:
                logger.error(f"✗ Household {household_id} has no Discord channel configured")
                logger.error("   Set discord_channel_id in household record")
                return False
            
            logger.info(f"✓ Household found with Discord channel: {household.discord_channel_id}")
            ...
            for item in items:
                ...
                if price:
                    ...
                    message_lines.append(f"• {item_name}: {quantity} {unit} (€{item_total:.2f})")
                else:
            ...
            if not channel:
                try:
                    logger.debug(f"   Channel {channel_id} not in cache, fetching from Discord API...")
                    channel = await self._bot_instance.fetch_channel(channel_id)
                    logger.info(f"✓ Successfully fetched channel {channel_id} from Discord API")
                    logger.debug(f"   Channel name: {channel.name if hasattr(channel, 'name') else 'unknown'}")
                except discord.errors.Forbidden as e:
                    logger.error("=" * 60)
            ...
            logger.info(f"✓ Channel accessible: {channel.name if hasattr(channel, 'name') else 'unknown'}")
            ...
            try:
                ...
                logger.info(f"   Channel: {channel.name if hasattr(channel, 'name') else 'unknown'}")
                logger.info("=" * 60)
                ...
                try:
                    ...
                    logger.warning(f"⚠ Could not add reactions: Bot lacks 'Add Reactions' permission")
                    logger.warning(f"   Error: {reaction_error}")
                except Exception as reaction_error:
                    logger.warning(f"⚠ Could not add reactions to message: {reaction_error}")
    ...
    async def _handle_order_button_response(
        ...
        try:
            ...
            if not user:
                logger.debug(f"User not found by Discord ID, trying to find by channel and name...")
                # Try to find by channel and name
                from models.household import Household
                household = await Household.find_one(Household.discord_channel_id == channel_id)
                if household:
                    logger.debug(f"Found household {household.household_id} for channel {channel_id}")
                    users = await User.find(
                        User.household_id == household.household_id,
                        User.is_active == True
                    ).to_list()
                    logger.debug(f"Found {len(users)} users in household, matching by name...")
                    for u in users:
                        if u.name.lower() == user_name.lower():
                            user = u
                            logger.info(f"✓ Matched user by name: {u.name} (User ID: {u.user_id})")
                            if not u.discord_user_id:
                                u.discord_user_id = discord_user_id
                                await u.save()
                                logger.info(f"✓ Updated user {u.user_id} with Discord ID: {discord_user_id}")
                            break
            
            if not user:
                logger.error(f"✗ User not found for Discord user {user_name} (ID: {discord_user_id})")
                logger.error("   User needs to have their Discord user ID set in their profile")
                await interaction.followup.send(
            ...
            if updated_order:
                ...
                logger.debug("Relaying response to agent...")
                try:
                    ...
                    logger.debug("✓ Response relayed to agent")
                except Exception as e:
                    logger.warning(f"⚠ Failed to relay response to agent: {e}")
            ...
            try:
                ...
                logger.error(f"✗ Also failed to send error message to user: {followup_error}")
